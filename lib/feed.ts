...
import { movieReleaseDate } from "./movie.ts";
...
/**
 * 映画を「公開日」の終日イベントとしてカレンダーへ登録する。
 * 映画はTV放送枠を持たないため、放送日ではなく公開日（released_on, 無ければseason近似）を使う。
 */
function movieToEvents(work: WorkDetail, opts: FeedOptions): IcsEvent[] {
  const date = movieReleaseDate(work);
  if (!date) return [];
  // 過去すぎる公開作品は出さない（窓: 過去45日〜未来約1.5年）。
  const t = new Date(`${date}T00:00:00+09:00`).getTime();
  const now = Date.now();
  if (t < now - PAST_DAYS * 86400000 || t > now + 550 * 86400000) return [];

  const lines: string[] = [];
  lines.push(work.releasedOn ? `公開日: ${date}` : `公開予定: ${work.releasedOnAbout ?? date}`);
  if (opts.includeUrl) lines.push(`作品ページ: ${appUrl()}/works/${work.id}`);
  lines.push("（この予定は アニメ放送カレンダー の購読フィードから配信されています）");

  return [
    {
      uid: `anidb-movie-${work.id}@anime-calendar`,
      startISO: `${date}T00:00:00+09:00`, // 並び替え用
      endISO: `${date}T00:00:00+09:00`,
      allDayDate: date,
      summary: work.releasedOn ? work.title : `${work.title}（公開予定）`,
...
function workToEvents(
  ...
  // 映画は放送枠ではなく公開日で登録する
  if (work.media === "movie") return movieToEvents(work, opts);
  const now = Date.now();
