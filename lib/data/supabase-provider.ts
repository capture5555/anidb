...
export class SupabaseDataProvider implements DataProvider {
  ...
  async listWorks(query: WorkQuery): Promise<WorkListResult> {
    ...
    // released_on / released_on_about は migration 0016 で追加。本番DBに未適用でも
    // 一覧が真っ白にならないよう、エラー時はこれらを外した select でフォールバックする。
    const SELECT_BASE =
      "id, title, title_kana, key_visual_url, poster_url, season_year, season_name, status, media, popularity, created_at, programs(count), work_genres(genres(name))";
    const SELECT_WITH_RELEASE = SELECT_BASE.replace(
      ", programs(count)",
      ", released_on, released_on_about, programs(count)",
    );

    const buildQuery = (select: string) => {
      let q = this.db.from("works").select(select, { count: "exact" });
    ...
    let hasRelease = true;
    let res = await buildQuery(SELECT_WITH_RELEASE);
    if (res.error) {
      hasRelease = false;
      res = await buildQuery(SELECT_BASE);
    }
    const { data, error, count } = res;
    if (error) throw error;

    // media不明(null)は放送枠(programs)がある場合のみ残す
    const dataRows = (data ?? []) as any[];
    const programCount = (row: any): number => (Array.isArray(row.programs) ? row.programs[0]?.count ?? 0 : 0);
    const allowed = dataRows.filter((row: any) => mediaAllowedInList(row.media, programCount(row) > 0));
    const removed = dataRows.length - allowed.length;
    ...
    const items: WorkSummary[] = rows.map((row: any) => ({
      ...
      releasedOn: hasRelease ? row.released_on ?? null : null,
      releasedOnAbout: hasRelease ? row.released_on_about ?? null : null,
  ...
  async getWork(id: string): Promise<WorkDetail | null> {
    ...
    return {
      ...
      releasedOn: w.released_on ?? null,
      releasedOnAbout: w.released_on_about ?? null,
      episodes: (w.episodes ?? [])
        .map((e: any) => ({
          id: e.id,
          workId: e.work_id,
