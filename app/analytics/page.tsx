...
export default async function AnalyticsPage({
  ...
  // AIコメント履歴（season スコープ）。2件以上あればステッパーで過去を辿れる。
  const { year: _y, season: _s } = seasonOf(new Date());
  const seasonLabel = formatSeason(_y, _s);
  const aiCommentHistory = await getAiCommentHistory("season", seasonLabel, 10).catch(() => []);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      ...
      {(seasonComment || aiCommentHistory.length > 0) && (() => {
        // ステッパー用 items を組み立てる: history 優先、なければ snapshot から1件
        const stepperItems =
          aiCommentHistory.length > 0
            ? aiCommentHistory.map((c) => ({
                body: c.body,
                generatedAt: c.generatedAt,
                title: c.title ?? undefined,
              }))
            : seasonComment
              ? [{ body: seasonComment.text, generatedAt: seasonComment.generatedAt ?? "", title: undefined }]
              : [];
        ...
        return (
          <div className="card p-4 sm:p-5 mb-5 border-l-4 border-l-accent">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              ...
              <span className="text-[0.66rem] text-muted">Grok・X / {seasonLabel}</span>
              <Link
