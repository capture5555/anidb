...
import { getVoiceActorDetail, type VoiceActorDetail, type PersonYearStat, type PersonWork } from "@/lib/analytics/people";
...
export async function generateMetadata({
  ...
  const detail = await getVoiceActorDetail(decoded).catch(() => null);
  const displayName = detail?.name ?? decoded;
  return { title: `${displayName} — 声優分析` };
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      ...
      <div className="space-y-5 py-5">
        ...
        {/* 参加作品一覧 */}
        <section className="card p-5 sm:p-6">
          <h2 className="section-title text-lg mb-1">参加作品一覧</h2>
          <p className="text-xs text-muted mb-4">
            出演作品（新しいクール順）。全{detail.worksCount}本。
          </p>
        ...
        {(detail.coActors.length > 0 || detail.coStaff.length > 0) && (
          <section className="card p-5 sm:p-6">
            ...
            <p className="text-xs text-muted mb-4">同じ作品に多く出演している声優・スタッフ。</p>
        ...
        <p className="text-xs text-muted leading-relaxed">
          ※ 声優の同定はクレジット表記のテキストに基づく近似であり、表記ゆれ等で精度に限界があります。
    ...
    <li className="flex items-center gap-3 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-muted tabular-nums">
          ...
          {w.isLead && (
            <span className="ml-2 text-accent font-bold text-[0.65rem]">主演</span>
          )}
        </p>
      </div>
