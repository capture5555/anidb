...
async function generateDailyNews(): Promise<void> {
  ...
  const query =
    ...
    `回答の**最後の1行**に、必ず以下の機械可読フォーマットで JSON 配列だけを出力してください:\n` +
    `NEWS_JSON: [{"title":"件名","summary":"説明文","url":"https://...（無ければ空文字）"}]\n` +
    `★ title と summary は必ず別の内容にすること（同じ文を繰り返さない）。\n` +
    `  - title: 25文字以内の短い件名（体言止め。例「『○○』TVアニメ化決定」）。\n` +
    `  - summary: その件を具体的に説明する1文（50〜100文字。いつ・誰が・何を、を含め、titleの言い換えにしない）。\n` +
    `JSON 配列は必ず6〜8要素。title・summary は日本語のみ。url は実在ページの https:// URL か空文字。` +
